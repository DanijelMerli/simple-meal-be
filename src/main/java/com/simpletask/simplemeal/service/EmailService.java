...
public class EmailService implements IEmailService {
	...
	public void sendChosenOneEmail(User chosenOne, Date today) {
		...
		for (OrderItem item : orderForToday.getOrderItems()) {
			emailBody.append("- ").append(item.getMeal().getName()).append(": ").append(item.getMealCount())
					.append(" (").append(item.getRegularMealSize()).append(")<br/>");
		}
		emailBody.append("<br/>");
		emailBody.append("Total Price: $").append(orderForToday.getTotalPrice()).append("<br/><br/>");
		emailBody.append(
				"This is the link for the checklist: <a href='http://localhost:4200/checklist'>Click here to view the checklist</a><br/><br/>");
	...
	public void sendDailyEmail(String to) {
		...
		try {
			...
			helper.setText("<html><body>This is the link for the daily menu: <a href='" + linkUrl + "'>" + linkText
					+ "</a></body></html>", true);
			javaMailSender.send(message);
		} catch (MessagingException e) {
			e.printStackTrace();
